...
class net:
    def __init__(self, hLayers, size):
        ...
        self.zScore = [[0 for i in range(0)]for l in range(x)]
        for l in range(0, x):
            for j in range(size[l]):
                self.activations[l].append(0)
                self.zScore[l].append(0)
    ...
    def activate(self, inputs):
        ...
        for j in range (len(inputs)):
            self.activations[0][j] = sigmoid(inputs[j])
            self.zScore[0][j] = inputs[j]
        for l in range (1, len(self.weights)):
            for j in range(len(self.activations[l])):
                self.zScore[l][j] = np.dot(self.weights[l][j], self.activations[l - 1])
                self.activations[l][j] = sigmoid(self.zScore[l][j])
        return self.activations[len(self.activations) - 1]
    def zActivate(self, inputs):
        #set input activations
        for j in range (len(inputs)):
            self.activations[0][j] = sigmoid(inputs[j])
            self.zScore[0][j] = inputs[j]
        for l in range (1, len(self.weights)):
            for j in range(len(self.activations[l])):
                self.zScore[l][j] = np.dot(self.weights[l][j], self.activations[l - 1])
                self.activations[l][j] = sigmoid(self.zScore[l][j])
        return self.zScore[len(self.zScore) - 1]

    def learn(self, inputs, outputs):
        #output errors
        for i in range(len(inputs)):
            #output layer
            outs = self.zActivate(inputs[i])
            error = np.subtract(outs, outputs[i])
            devSig = abs(np.multiply(outs, np.subtract(1, outs)))
            error = np.multiply(error, devSig)
            self.errors[len(self.activations) - 1] = np.multiply(error, 2)
            #rest of layers
